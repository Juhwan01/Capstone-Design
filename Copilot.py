...
def prepare_code_for_execution(code: str) -> tuple:
    """Prepare the code for execution by replacing input() calls with placeholders."""
    input_pattern = r"input\s*\((.*?)\)"
    matches = re.findall(input_pattern, code)
    
    for i, match in enumerate(matches):
        placeholder = f"__INPUT_PLACEHOLDER_{i}__"
        code = code.replace(f"input({match})", f"'{placeholder}'")
    
    return code, matches

def execute_code(code: str, user_inputs: list) -> str:
    """Execute the given code with user inputs and return the output."""
    if not is_safe_code(code):
        return "Error: This code contains potentially unsafe operations and cannot be executed."

    prepared_code, input_prompts = prepare_code_for_execution(code)
    ...
    try:
        ...
        for i, user_input in enumerate(user_inputs):
            prepared_code = prepared_code.replace(f"'__INPUT_PLACEHOLDER_{i}__'", f"'{user_input}'")
...
def main():
    ...
    with col1:
        ...
        # Prepare code and get input prompts
        prepared_code, input_prompts = prepare_code_for_execution(code)
        
        # Create input fields for each input() call in the code
        user_inputs = []
        for i, prompt in enumerate(input_prompts):
            user_input = st.text_input(f"Input {i+1}: {prompt.strip()}")
        ...
        with col1_2:
            if st.button("Execute Code"):
                if code:
                    output = execute_code(code, user_inputs)
                    st.session_state.execution_output = output
                else:
                    st.warning("Please enter some code first.")
